...
export type Toast = {
    show?: boolean;
    position?: "top-right" | "top-left" | "bottom-right" | "bottom-left" | "none" | undefined; 
    message?: string, 
...
export interface Toastable<T>  {
    ...
    set: (data: T) => void;
    update: (data: T) => void;
    show: (message: string, type?: "success" | "error" | "info", duration?: number, position?: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left' | 'none' | undefined) => void;
...
export const toastable = ():Toastable<Toast> => {
    ...
    return {
        show: (message: string, type:"success" | "error" | "info", duration: number = 5000, position: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left' | 'none' | undefined = 'top-right') => {
            let color = "green";
            switch(type) {
                case "success":
                    color = "green";
